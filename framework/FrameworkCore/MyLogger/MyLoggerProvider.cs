...
namespace FrameworkCore.MyLogger
{
    ...
    {
        readonly MyLoggerSettings _configuration;
        readonly ConcurrentDictionary<string, InitLoggerModel> _loggerKeys = new ConcurrentDictionary<string, InitLoggerModel>();
        readonly ConcurrentDictionary<string, MyLogger> _loggers = new ConcurrentDictionary<string, MyLogger>();

        public MyLoggerProvider(MyLoggerSettings configuration)
        {
        ...
        public ILogger CreateLogger(string categoryName)
        {
            ...
            {
                var logger = new MyLogger(categoryName);
                InitLogger(loggerKey, logger);
                return logger;
            });
        }

        private static void InitLogger(InitLoggerModel model, MyLogger logger)
        {
